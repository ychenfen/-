...
\usepackage{listings}
\usepackage{xcolor}
% \usepackage{fontawesome5} % 暂时禁用图标包
\usepackage{multicol}
...
\begin{frame}
    ...
    \begin{center}
        ...
        \begin{tikzpicture}
            ...
            \node[fill=warningyellow, text=white, rounded corners=3pt, inner sep=5pt] at (5,0) {\footnotesize ☁️ 边缘计算};
...
\begin{frame}{系统价值}
    \begin{columns}
        \begin{column}{0.5\textwidth}
            \begin{block}{\faBalanceScale\ 教育公平性}
                \begin{itemize}
                    \item 为偏远地区提供优质教育资源
                    \item 24/7全天候智能教学服务
                    \item 降低优质教育获取门槛
...
\begin{frame}{RISC-V边缘AI计算}
    \begin{columns}
        \begin{column}{0.6\textwidth}
            \begin{block}{硬件平台}
                \begin{itemize}
                    \item \faChip\ 8核RISC-V处理器@2.0GHz
                    \item \faBrain\ 集成NPU，2.0 TOPS算力
                    \item \faMemory\ 8GB LPDDR5 + 256GB存储
                    \item \faBatteryHalf\ 典型功耗<20W
                \end{itemize}
            \end{block}
...
\begin{frame}{机械臂可视化控制}
    \begin{columns}
        ...
        \begin{column}{0.5\textwidth}
            \begin{block}{技术规格}
                \begin{itemize}
                    \item \faRobot\ myCobot 280 6轴机械臂
                    \item \faRuler\ 重复定位精度：±0.5mm
                    \item \faWeight\ 负载能力：250g
                    \item \faClock\ 响应时间：<500ms
                \end{itemize}
            \end{block}
...
\begin{frame}{实时视觉感知系统}
    \begin{columns}
        ...
        \begin{column}{0.4\textwidth}
            \begin{block}{视觉检测}
                \begin{itemize}
                    \item \faVideo\ 1280x720@30fps
                    \item \faSearch\ 实时目标检测
                    \item \faEye\ OCR文字识别
                    \item \faAdjust\ 置信度阈值调节
                \end{itemize}
            \end{block}
...
\begin{frame}{教学效果评估}
    \begin{columns}
        ...
        \begin{column}{0.4\textwidth}
            \begin{block}{测试设计}
                \begin{itemize}
                    \item \faUsers\ 20名小学生(8-12岁)
                    \item \faBook\ 数学基础运算
                    \item \faClock\ 每次30分钟，连续3天
                    \item \faCompare\ 传统 vs AI教学对比
                \end{itemize}
            \end{block}
...
\begin{frame}{技术创新突破}
    \begin{columns}
        \begin{column}{0.5\textwidth}
            \begin{block}{\faRocket\ RISC-V生态创新}
                \begin{itemize}
        ...
        \begin{column}{0.5\textwidth}
            ...
            \begin{block}{\faGraduationCap\ 教育应用创新}
                \begin{itemize}
                    \item K12教育深度定制
                    \item 个性化教学策略
                    \item 实时学习效果评估
...
\begin{frame}{市场竞争优势}
    \begin{table}[h]
        ...
        \hline
        多模态融合 & \success{✓ 四模态} & \danger{✗ 双模态} & \warning{△ 三模态} & \danger{✗ 单模态} \\
        \hline
        具身交互 & \success{✓ 机械臂} & \danger{✗ 无} & \danger{✗ 无} & \warning{△ 简单} \\
        \hline
        隐私保护 & \success{✓ 边缘计算} & \danger{✗ 云端} & \danger{✗ 云端} & \success{✓ 本地} \\
        \hline
        响应时间 & \success{<2秒} & 3-5秒 & 2-4秒 & 1-3秒 \\
        \hline
        教育专用性 & \success{✓ 深度定制} & \danger{✗ 通用} & \warning{△ 部分} & \danger{✗ 通用} \\
        \hline
...
\begin{frame}{项目成果总结}
    \begin{columns}
        \begin{column}{0.5\textwidth}
            \begin{block}{\faCheckCircle\ 技术成果}
                \begin{itemize}
                    \item RISC-V边缘AI教育系统
                    \item 四模态实时融合技术
                    \item 机械臂教学手势控制
...
\begin{frame}{未来展望}
    \begin{center}
        \begin{tikzpicture}[scale=1.1]
            ...
            % 周围应用场景
            \foreach \angle/\text/\color in {
                0/个性化学习/successgreen,
                60/教育公平/warningyellow,
                120/师资增强/dangerred,
                180/创新教学/primaryblue,
                240/终身学习/secondaryblue,
                300/全球教育/darkgray
            } {
                \draw[thick, \color] (0,0) -- (\angle:2.5);
                \node[fill=\color!20, rounded corners=3pt, text width=1.8cm, align=center] at (\angle:3.2) {\footnotesize\text};
            }
            
            % 技术支撑
            \node[below] at (0,-4) {
                \begin{tabular}{cccc}
                    \faChip & \faBrain & \faRobot & \faGraduationCap \\
                    RISC-V & 多模态AI & 具身智能 & 智能教育 \\
                \end{tabular}
            };
        \end{tikzpicture}
    \end{center}
